...
function der_CJCKD4(n, dn_dgamma, no1, ndoc, nalpha, nv, nbf5, ndns, ncwo)
    ...
    for k = 1:nv
        ...
        D_Pi[1:ndoc, ndoc+1:end, k] = -(
            ...
            sqrt.((n[1:ndoc] .- n[ndoc+1:end]') .+ R) .+
            sqrt.(R) .* (
                (dn_dgamma[1:ndoc, k] .- dn_dgamma[ndoc+1:end, k]') .+ D_R[1:end, 1:end, k]
...
function der_CJCKD8(
    ...
    elseif ista == 3
        ...
        for i = no1+1:nbf5
            a = max(fi[i], 10^-15)
            for k = 1:nv
                dfi_dgamma[i, k] = 1 / (2 * a) * (1 - 2 * n[i]) * dn_dgamma[i, k] * exp(-(n[i] - 0.5)^2)
                dfi_dgamma[i, k] += - fi[i] * 2 * (n[i] - 0.5) * dn_dgamma[i, k] * exp(-(n[i] - 0.5)^2)
            end
        end
        fi = fi .* exp.( - (n .- 0.5) .^2 )
    elseif ista == 4
